...
\begin{enumerate}[label=\textbf{\textbullet}, align=left, leftmargin=*]
	\item[\textbf{Definitionen}] { \scriptsize \mbox{\textbf{KA}: Klassenarbeit}; \mbox{\textbf{GFS}: gleichwertige Feststellung von Leistungen der Schüler}; \mbox{\textbf{KT}: schriftliche Wiederholungsarbeit}; \linebreak \mbox{\textbf{P}: praktische Arbeit}; \mbox{\textbf{m}: mündlich} ; \textit{Notensysteme}: Noten (\textbf{N}), Notenpunkte (\textbf{NP}) }
	\item[\textbf{Arithmetische Mittelwerte der Noten}] $\mkt$ (KT und P); $\mka$ (KA und GFS); $\mm$ (m)
