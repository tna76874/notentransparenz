...
\begin{enumerate}[label=\textbf{\textbullet}, align=left, leftmargin=*]
	...
	\vspace*{-12pt}
	\item[\textbf{Gewichtungsfaktor KA/KT}] $w_{s_0}$; $n_{{KT}_0}$  (Falls nicht anders mitgeteilt, ist $w_{s_0}=1$ und $n_{{KT}_0}=2$)
	\[
	w_s =
	\begin{cases}
		0 & \text{für }  n_{KT} = 0 \\
		\frac{n_{KT} \cdot w_{s_0}}{n_{{KT}_0}} & \text{für }    n_{KT} < n_{{KT}_0} \\
		w_{s_0} & \text{für }    n_{KT} \geq n_{{KT}_0} \\
	\end{cases}
	\]	
